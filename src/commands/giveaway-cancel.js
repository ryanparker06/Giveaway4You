  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("message_id")
        .setDescription(
          "The message ID of the giveaway to cancel"
        )
        .setRequired(true)
    ),
  ...
  async execute(interaction) {
    ...
    if (!isAdministrator) {
      ...
      if (!hasAccess) {
        return interaction.reply({
          ...
          content:
            "❌ Only server administrators or users granted giveaway access can cancel giveaways."
        });
      }
    }
    ...
    if (giveaway.ended) {
      return interaction.reply({
        flags: MessageFlags.Ephemeral,
        content:
          "⚠️ This giveaway has already ended or been cancelled."
      });
    }
