...
// Sends a styled notification to your log channel whenever Giveaway4You is removed from a server.
  ...
  async execute(guild, client) {
    try {
      ...
      const header = new SectionBuilder()
        .addTextDisplayComponents(
          new TextDisplayBuilder().setContent(
            `# ❌ Giveaway4You Removed from a Server`
          )
        )
      ...
      const details = new TextDisplayBuilder().setContent(
        ...
        `> **Server ID:** ${guild.id}\n` +
        `> **Member Count:** ${guild.memberCount?.toLocaleString() || "Unknown"}\n` +
        `> **Owner ID:** ${guild.ownerId || "Unknown"}\n` +
        `> **Shard:** #${guild.shardId ?? 0}\n\n` +
        `📉 Giveaway4You is now in **${client.guilds.cache.size.toLocaleString()} servers.**`
      );
